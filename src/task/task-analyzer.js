...
class TaskAnalyzer {
     ...
     * 评估复杂度 (1-5)
     */
    assessComplexity(prompt, files, keywords) {
        let score = 2; // 基础分
        // 文件数量影响
        if (files.length > 5)
            score += 2;
            ...
            score += 1;
        // 高复杂度关键词
        if (COMPLEXITY_INDICATORS.high.some(k => prompt.toLowerCase().includes(k)))
            score += 1;
        // 低复杂度关键词
        if (COMPLEXITY_INDICATORS.low.some(k => prompt.toLowerCase().includes(k)))
            score -= 1;
        // prompt 长度影响
     ...
     * 判断是否可拆分
     */
    isSplittable(prompt, category, complexity) {
        // 复杂度高的任务通常可拆分
        if (complexity >= 4)
            return true;
        // 包含多个动作的任务可拆分
        const actionWords = ['和', '并且', '同时', 'and', 'also', '以及', '还要'];
        if (actionWords.some(w => prompt.includes(w)))
            return true;
        // 架构类任务通常可拆分
