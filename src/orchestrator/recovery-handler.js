...
class RecoveryHandler {
    ...
    taskStateManager;
    config;
    constructor(cliFactory, snapshotManager, taskStateManager, config) {
     ...
     * 执行恢复流程
     */
    async recover(taskId, failedTask, verificationResult, errorDetails) {
        const attempts = failedTask.attempts;
        console.log(`[RecoveryHandler] 开始恢复流程，当前尝试次数: ${attempts}`);
        events_1.globalEventBus.emitEvent('recovery:started', {
            taskId,
        ...
        events_1.globalEventBus.emitEvent('recovery:completed', {
            taskId,
            data: result
        });
        return result;
     ...
     * 确定恢复策略
     */
    determineStrategy(attempts) {
        if (attempts < 1)
            return 'retry_same_cli';
        if (attempts < 2)
            return 'retry_with_context';
        if (attempts < this.config.maxAttempts)
            return 'escalate_to_claude';
        return 'rollback';
    }
    /**
     * Strike 1: 原 CLI 尝试修复
     */
    async retrySameCli(taskId, failedTask, errorDetails) {
     ...
     * Strike 2: 提供更多上下文重试
     */
    async retryWithContext(taskId, failedTask, errorDetails) {
     ...
     */
    async escalateToClaude(taskId, failedTask, errorDetails) {
     ...
     * 超过最大重试次数：执行回滚
     */
    async performRollback(taskId, failedTask) {
     ...
     * 构建修复 Prompt
     */
    buildFixPrompt(task, errorDetails, level) {
        if (level === 'simple') {
            return `
之前的任务执行失败，请修复以下错误：
...
`.trim();
     ...
     */
    buildEscalatePrompt(task, errorDetails) {
        return `
...
`.trim();
     ...
     * 检查是否应该继续恢复
     */
    shouldContinueRecovery(task) {
        return task.attempts < this.config.maxAttempts;
    }
    /**
     * 获取恢复统计
     */
    getRecoveryStats(tasks) {
...
//# sourceMappingURL=recovery-handler.js.map